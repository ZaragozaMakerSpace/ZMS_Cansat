...
void radioInfo() {
  ...
  String strPaquete = datosRadio(); //Crea el paquete completo con la información;
  char radiopacket[64];//Tenemos una cadena de texto

  sprintf(radiopacket, "%s", strPaquete.c_str()); //Metemos en la cadena de texto la parte que nos interesa del mensaje(tipo String)
  Serial.print("enviando ");
  Serial.print(radiopacket);

  // Enviar el mensaje al DESTINO (DEST_ADDRESS)

  rf69_manager.sendto((uint8_t *)radiopacket, strlen(radiopacket), DEST_ADDRESS);
  setupDatos();
}
